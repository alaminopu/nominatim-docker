FROM ubuntu:latest
MAINTAINER Al-Amin <alaminopu.me@gmail.com>

# Define the OSM argument, use monaco as default
ARG OSM=http://download.geofabrik.de/asia/bangladesh-latest.osm.pbf
...
RUN apt-get  -y install sudo vim libboost-all-dev libboost-dev libboost-system-dev python3-dev python3-pip python3-psycopg2
...
RUN mkdir -p /app/git/
RUN git clone --recursive https://github.com/openstreetmap/Nominatim.git /app/git/
